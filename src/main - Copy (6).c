...
#define MAX_PERIOD PWM_MSEC(5U)    // Start with 0.05 seconds (50,000,000 ns) to reduce    // Maximum period (1 second)
...
static struct {
    ...
    uint8_t count;
    bool active;
} pulse_queue[7]; // Support up to 7 LEDs (RED, GREEN, BLUE, YELLOW, ORANGE, MAGENTA, WHITE)
static uint8_t queue_index = 1;

static uint32_t max_period = MAX_PERIOD;
static volatile bool pulsing = false;
static uint32_t total_execution_ms; // Total time including grace period
static uint8_t remaining_pulses;   // Remaining pulse cycles
static enum led_color pulse_color = LED_OFF;
static const uint32_t grace_period_ms = 250; 
static uint32_t pulse_duration_ms; // Total duration for one pulse (rising + falling)
static uint32_t total_steps;       // Calculated steps for the pulse    
static uint8_t current_step;       // Current step in the sequence
static uint32_t total_execution_ms; // Total time including grace period
uint8_t max_steps; // Number of fade steps
...
void pulse_led(enum led_color color, uint32_t duration_ms, uint8_t pulse_count) {
    if (pulsing && queue_index >= 7) return; // Queue full, drop call

    LOG_INF("Queuing pulse: color=%d, duration=%d ms, count=%d", color, duration_ms, pulse_count);

    // Store in queue
    pulse_queue[queue_index].color = color;
    pulse_queue[queue_index].duration_ms = duration_ms; // Each pulse duration
    pulse_queue[queue_index].count = pulse_count;
    pulse_queue[queue_index].active = false;
    queue_index++;

    // Start the first pulse if not already pulsing
    if (!pulsing) {
        pulsing = true;
        pulse_color = pulse_queue[0].color;
        pulse_duration_ms = pulse_queue[0].duration_ms; // Duration of each pulse
        pulse_count = pulse_queue[0].count;
        remaining_pulses = pulse_count;
        current_step = 0;

        // Calculate total steps per pulse (rising + falling)
        total_steps = (pulse_duration_ms + (2 * step_delay - 1)) / (2 * step_delay);
        if (total_steps < 10) total_steps = 10; // Minimum for smoothness
        LOG_INF("Calculated total_steps=%d per pulse for duration %d ms", total_steps, pulse_duration_ms);

        total_execution_ms = pulse_duration_ms * pulse_count + grace_period_ms;
        LOG_INF("Total execution time including grace period: %d ms", total_execution_ms);
...
void pulse_timer_handler(struct k_timer *timer) {
    ...
    // Calculate total steps for all pulses
    uint32_t total_pulse_steps = total_steps * 2; // Rising + falling per pulse

    // Determine current pulse and step within it
    ...
    if (current_pulse < pulses) {
        if (step_in_pulse < total_steps) {
            // Rising phase: 0 to max_period (off to on) for current pulse
            ...
            // Falling phase: max_period to 0 (on to off) for current pulse
    ...
    } else if ((uint32_t)(current_step - (pulses * total_pulse_steps)) * step_delay < grace_period_ms) {
        // Grace period: keep LEDs off, maintain pulsing state
        red_pulse = 0;
        green_pulse = 0;
        blue_pulse = 0;
        if (current_step == pulses * total_pulse_steps) {
            LOG_INF("Entering grace period of %d ms", grace_period_ms);
        }
    } else {
        // Grace period complete, move to next in queue or stop
        pulsing = false;
        current_step = 0;
        pwm_set_dt(&pwm_red, max_period, 0); // Ensure off
        pwm_set_dt(&pwm_green, max_period, 0);
        pwm_set_dt(&pwm_blue, max_period, 0);
        k_timer_stop(&pulse_timer);
        LOG_INF("Pulse cycle complete, system unlocked");

        // Shift queue and start next pulse if available
        if (queue_index > 0) {
...
void battery_pulse_timer_handler(struct k_timer *timer) {
    if (pulses < 8 && !is_charging) {
        pulse_led(LED_RED, 1000,3); // Fade in/out over 1s
        pulses++;
    } else {
        pulses = 0;
...
static void charging_pulse_timer_handler(struct k_timer *timer) {
    if (is_charging) {
        ...
        pulse_led(LED_RED, 3000,1); // 3 pulses of 1.5s each
...
// Prepare 8-byte payload
    ...
    if (err == 0) {
        ...
        pulse_led(LED_GREEN, 400,2); // Replace blink
    } else {
        pulse_led(LED_RED, 300,2); // Replace blink
    }
    set_led_status(LED_OFF);
}
void sample_and_transmit(struct esb_evt const *event) {
 ...
 */// PWM Calibration
   // PWM Calibration
    ...
    pwm_set_dt(&pwm_green, max_period, 0);
    pwm_set_dt(&pwm_blue, max_period, 0);
    LOG_INF("ALL LEDS INITIALIZED OFF RED: %d, GREEN: %d, BLUE: %d (0 duty cycle)", 0, 0, 0);
    //k_timer_start(&led_timer, K_SECONDS(5), K_NO_WAIT);
 ...
 /*    gpio_pin_interrupt_configure_dt(&data_plus, GPIO_INT_EDGE_BOTH);
    ...
    LOG_INF("BLUE LED should be ON (100 duty cycle)");
    set_led_status(LED_BLUE);
    k_sleep(K_SECONDS(1));
    LOG_INF("BLUE LED should be OFF (0 duty cycle)");
    set_led_status(LED_OFF);

    LOG_INF("MAGENTA LED should be ON (100 duty cycle)");
    set_led_status(LED_MAGENTA);
    k_sleep(K_SECONDS(1));
    LOG_INF("MAGENTA LED should be OFF (0 duty cycle)");
    set_led_status(LED_OFF);

   while(i!=0) {
    check_battery_status(); 
    LOG_INF("RED LED should pulse ON/OFF for 1 second/3 times");
    pulse_led(LED_RED, 2000,3);
    LOG_INF("GREEN LED should pulse ON/OFF for 1 seconds/3 times");
    pulse_led(LED_GREEN, 2000,3);
    LOG_INF("BLUE LED should pulse ON/OFF for 1 seconds/3 times");
    pulse_led(LED_BLUE, 2000,3);
    i--;
    } 
    k_timer_start(&battery_check_timer, K_NO_WAIT, K_MINUTES(1));
