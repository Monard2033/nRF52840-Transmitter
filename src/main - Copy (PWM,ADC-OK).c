...
static K_TIMER_DEFINE(pulse_timer, NULL, NULL); // Timer for pulsing
...
void pulse_timer_handler(struct k_timer *timer) {
    ...
    uint32_t period = 20000; // 20 kHz period
    uint32_t red_pulse = period, green_pulse = period, blue_pulse = period;

    if (pulse_step < max_steps) {
        // Fade in: Increase duty cycle from period to 0 (inverted)
        uint32_t duty = period - (pulse_step * (period / max_steps));
        switch (pulse_color) {
            ...
            case LED_ORANGE:
                red_pulse = duty;
                green_pulse = duty + (period / 2); // Adjust for dimming
                break;
            case LED_WHITE:
                red_pulse = duty;
                green_pulse = duty;
        ...
        // Fade out: Decrease duty cycle from 0 to period
        uint32_t duty = ((pulse_step - max_steps) * (period / max_steps));
        switch (pulse_color) {
            case LED_RED: red_pulse = duty; break;
            case LED_GREEN: green_pulse = duty; break;
            case LED_BLUE: blue_pulse = duty; break;
        ...
        pulsing = false;
        pulse_step = 0;
        pwm_set_dt(&pwm_red, period, period); // Ensure off
        pwm_set_dt(&pwm_green, period, period);
        pwm_set_dt(&pwm_blue, period, period);
        k_timer_stop(&pulse_timer);
        return;
    }

    pwm_set_dt(&pwm_red, period, red_pulse);
    pwm_set_dt(&pwm_green, period, green_pulse);
    pwm_set_dt(&pwm_blue, period, blue_pulse);
    pulse_step++;
...
void pulse_led(enum led_color color, uint32_t duration_ms, uint8_t pulse_count) {
    ...
    // Ensure duration_ms is respected by adjusting pulse_count or step_delay if needed
}
...
int check_battery_status(void) {
    ...
    if (k_uptime_get() < Z_TIMEOUT_MS_TICKS(5000)) {
        ...
        //blink_led(LED_WHITE , 500,5); // Turn off after 5 seconds
    }
 ...
 */
    if (!device_is_ready(pwm_red.dev)) {
        ...
        blink_led(LED_RED, 100, 5); // Fallback using GPIO if defined
    }
    LOG_INF("PWM initialized for RGB LEDs: P0.22 (Red), P0.24 (Green), P1.00 (Blue)");
    //k_timer_start(&led_timer, K_SECONDS(5), K_NO_WAIT);
 ...
 /*    gpio_pin_interrupt_configure_dt(&data_plus, GPIO_INT_EDGE_BOTH);
    ...
    //blink_led(LED_BLUE, 250, 3);

    check_battery_status();
    set_led_status(LED_WHITE);
